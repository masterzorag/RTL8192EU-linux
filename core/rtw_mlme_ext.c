	...
	if (b5GBand) {
		...
		for (index = 0; index < CH_LIST_LEN(RTW_ChannelPlan5G[Index5G]); index++) {
			if (rtw_regsty_is_excl_chs(regsty, CH_LIST_CH(RTW_ChannelPlan5G[Index5G], index)) == _TRUE)
				continue;
			#ifndef CONFIG_DFS
			if (rtw_is_dfs_ch(CH_LIST_CH(RTW_ChannelPlan5G[Index5G], index), CHANNEL_WIDTH_20, HAL_PRIME_CHNL_OFFSET_DONT_CARE))
				continue;
			#endif
			...
			if ((ChannelPlan == RTW_CHPLAN_WORLD_WIDE_5G) /* all channels passive */
				...
				|| (rtw_is_dfs_ch(channel_set[chanset_size].ChannelNum
					, CHANNEL_WIDTH_20
					, HAL_PRIME_CHNL_OFFSET_DONT_CARE)) /* DFS channel(band2, 3) passive */
			)
				channel_set[chanset_size].ScanType = SCAN_PASSIVE;
			else
				channel_set[chanset_size].ScanType = SCAN_ACTIVE;
