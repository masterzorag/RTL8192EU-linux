...
)
{
	if (addr == 0xfe || addr == 0xffe) {
		...
		if (addr == 0xb6) {
			u32 getvalue = 0;
			u8	count = 0;
			getvalue = odm_get_rf_reg(p_dm_odm, RF_PATH, addr, MASKDWORD);
			...
			while ((getvalue >> 8) != (data >> 8)) {
				...
				ODM_delay_us(1);
				getvalue = odm_get_rf_reg(p_dm_odm, RF_PATH, addr, MASKDWORD);
				ODM_RT_TRACE(p_dm_odm, ODM_COMP_INIT, ODM_DBG_TRACE, ("===> odm_config_rf_with_header_file: [B6] getvalue 0x%x, data 0x%x, count %d\n", getvalue, data, count));
				if (count > 5)
					break;
			}
		}
		...
		if (addr == 0xb2) {
			...
			while (getvalue != data) {
				...
				ODM_delay_us(1);
				/* Do LCK againg */
				odm_set_rf_reg(p_dm_odm, RF_PATH, 0x18, RFREGOFFSETMASK, 0x0fc07);
				ODM_delay_us(1);
				getvalue = odm_get_rf_reg(p_dm_odm, RF_PATH, addr, MASKDWORD);
				ODM_RT_TRACE(p_dm_odm, ODM_COMP_INIT, ODM_DBG_TRACE, ("===> odm_config_rf_with_header_file: [B2] getvalue 0x%x, data 0x%x, count %d\n", getvalue, data, count));
				if (count > 5)
					break;
			}
		}
...
)
{
	...
	odm_config_rf_reg_8192e(p_dm_odm, addr, data, ODM_RF_PATH_A, addr | maskfor_phy_set);
...
)
{
	...
	odm_config_rf_reg_8192e(p_dm_odm, addr, data, ODM_RF_PATH_B, addr | maskfor_phy_set);
