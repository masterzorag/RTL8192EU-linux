		...
		*/
		...
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			if (!iface)
				continue;
			if (rtw_linked_check(iface) &&
			    check_fwstate(&(iface->mlmepriv), _FW_LINKED | WIFI_AP_STATE) != _TRUE) {
...
void	update_ra_mask_8192e(_adapter *padapter, struct sta_info *psta, struct macid_cfg *h2c_macid_cfg)
{
	u8	arg[4] = {0};
	...
	rtl8192e_set_raid_cmd(padapter, h2c_macid_cfg->ra_mask, arg, h2c_macid_cfg->bandwidth);
}
...
void rtl8192e_set_hal_ops(struct hal_ops *pHalFunc)
{
	...
	pHalFunc->update_ra_mask_handler = update_ra_mask_8192e;
